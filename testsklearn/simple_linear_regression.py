...
def generate_lr_train_data(polynomial = False):
    if not polynomial:
        f = open("../data/simple_lr.data", "w")
        for i in range(200):
            f.write("%s %s\n" % (i, i * 3 + np.random.normal(0, 50)))
    else:
        f = open("../data/polynomial_lr.data", "w")
        for i in range(200):
...
def read_lr_train_data(polynomial = False):
    if not polynomial:
        return pd.read_csv("../data/simple_lr.data", header = None)
    else:
        return pd.read_csv("../data/polynomial_lr.data", header = None)
